...
void RF2D::evalIC(double* values, const double* x) const
{
  ...
  //    values[0] = 10.0*1/(2.0*M_PI)*sqrt(1/(2.0*M_PI))*exp(-( (x[0]-0.0)*(x[0]-0.0) + (x[1]-0.0)*(x[1]-0.0))/0.2 ) ;
   ...
   values[0] = 10.0*1/(2.0*M_PI)*sqrt(1/(2.0*M_PI))*exp(-( (x[0]-0.0)*(x[0]-0.0) )/0.2 )*std::pow(x[0],2)  ;
...
void RF2D::evalDrift(double* values, const double* x) const
  ...
  long double Gp =( erf(x[0]) - 0.2e1*x[0] 
   ...
   values[0]= 1/(std::pow(vth, 2))*(   alpha + q01/(vth*x[0]+ 1e-39)*( 1 + std::pow(x[1],2))  )/ ( std::pow(x[0],2) + 1e-39) ; //A2
   values[1]= 1/(std::pow(vth, 2))*(  q01/( std::pow(x[0],2) + 1e-39 )*x[1]*(1 - 3.0*std::pow(x[1],2))
				      - 1/( 2.0*std::pow(x[0],2) + 1e-39 )*x[1]*gamma )
     /(std::pow(x[0],2) + 1e-39) ;//A3
...
void RF2D::evalDiffusion(double* values, const double* x) const
  {
   ...
   // m(x,xi) = \frac{1}{v_{th}^2} \left( \frac{\langle (\Delta v_{\perp})^2\rangle}{2  x^2 }( 1 - \xi^2) +   \frac{2 q_{0,1}}{ x^2}(1 + \xi^2) \right)
    ...
    long double g = 1/(std::pow(vth,2.0))*(  beta  + 4.0*q01 
					     - 2.0*q01*(  1.0 + std::pow(x[1],2.0)  ) ); 
    long double k = 2.0/(std::pow(vth, 2.0))*q01/(x[0]  + 1e-39);
    long double o = ( std::pow(x[1],2)  -1 )*x[1];
    long double m = 1/(std::pow(vth,2.0))*( gamma/( 2.0*std::pow(x[0],2.0) + 1e-39  ) 
					    *( 1.0 - std::pow(x[1], 2.0) )  
					    + 2.0*q01/( std::pow(x[0],2.0) + 1e-39  )
					    *std::pow(x[1],2)*( 1.0 - std::pow(x[1], 2.0) ) ); 
    
    long double m1 = gamma/( 2.0*std::pow(x[0],2.0) + 1e-39 )*( 1.0 - std::pow(x[1], 2.0) );
    long double m2 = 2.0*q01/( std::pow(x[0],2.0) + 1e-39 )*std::pow(x[1],2)*( 1.0 - std::pow(x[1], 2.0) );
    ...
    values[0] = sqrt(g)/(x[0] + 1e-39) ;     // B11
    values[1] = k*o/(sqrt(g)*x[0] + 1e-39); // B12
    values[2] = 0.0; //B21 
    values[3] = sqrt( m*g -k*k*o*o )/( sqrt(g)*x[0] + 1e-39); //B22
...
void RF2D::evalSource(double* val, const double* x) const {
  ...
  //  /(std::pow(M_PI, 3.0/2.0)*std::pow(vth, 5))*std::pow(x[0],2);
  ...
  val[0] = 2.0*exp(-std::pow(x[0],2))*(-3.0 + 2.0*std::pow(x[0],2) )
    /(std::pow(M_PI, 3.0/2.0))*std::pow(x[0],2);
